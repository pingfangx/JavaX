        ...
        if (element is XmlTag) {
            with(element) {
                ...
                if (name.equals("include", ignoreCase = true)) {
                    ...
                    val psiFiles = FilenameIndex.getFilesByName(project, layoutName + Constant.Action.SELECTED_TEXT_SUFFIX, GlobalSearchScope.allScope(project))
                    if (psiFiles.isNotEmpty()) {
                        include = if (psiFiles.size > 1) {
                            val psiFilePath = this@getIDsFromLayoutToList.parent?.toString()!!
